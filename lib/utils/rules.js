...
const evaluateRules = (criteria, req, options, userRules, log) => {
  ...
  Object.keys(criteria).forEach(key => {
    // there is no need to continue evaluating rules if any previous rule failed
    if (!allRulesMatched) { return }
    ...
    if (userRules[key]) {
      ...
      if (!splitterRules[key](criteria[key], req, options)) { allRulesMatched = false }
    } else {
      log.warn(`Callback to evaluate rule '${key}' not found. Rule ignored.`)
    }
  })

  if (!allRulesMatched) { return false }
