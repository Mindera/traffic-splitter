...
const evaluateRules = (criteria, req, options, userRules, log) => {
  ...
  Object.keys(criteria).forEach(key => {
    ...
    // first search for the key in userRules - this allows user to override splitter rules
    if (userRules[key]) {
      numRules++
      ...
      numRules++
      if (predefinedRules[key](criteria[key], req, options)) { rulesMatched++ }
    } else {
