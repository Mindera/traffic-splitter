...
const evaluateRules = (criteria, req, options, userRules, log) => {
  ...
  if (!Object.keys(criteria).every(key => {
    ...
    if (['and', 'or'].indexOf(key) >= 0) { return true }
  ...
  if (criteria.and && criteria.and.length > 0) {
    if (!criteria.and.map(single => evaluateRules(single, req, options, userRules, log)).every(match => match)) {
      return false
    }
  }
  ...
  if (criteria.or && criteria.or.length > 0) {
    if (!criteria.or.map(single => evaluateRules(single, req, options, userRules, log)).some(match => match)) {
      return false
    }
  }
