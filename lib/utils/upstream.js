...
const determineUpstream = ({rulesets, upstreams}, eventEmitter, userRules, logger) => (req, res, next) => {
  ...
  const evaluateRuleset = (setName) => {
    if (!rulesets[setName]) {
      logger.warn(`Ruleset with name '${setName}' not found. Ruleset ignored.`)
      return true // ignore ruleset
    }

    if (rulesetsEvaluated[setName] === undefined) {
      rulesetsEvaluated[setName] = rules.evaluateRules(rulesets[setName], req, options, userRules, logger)
    }
  ...
  upstreams.some(upstream => {
    ...
    if (setsMatched.out && upstream.criteria.out && rules.evaluateRules(upstream.criteria.out, req, options, userRules, logger)) {
      return false
    }

    if (setsMatched.in && rules.evaluateRules(upstream.criteria.in, req, options, userRules, logger)) {
