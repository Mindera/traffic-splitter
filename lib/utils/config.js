...
const getOptimizedUpstreamOptions = ({type, options}) => {
  ...
  if (type === 'loadbalancer') {
    const opts = Object.assign({}, options, {healthcheck: options.healthcheck ? options.healthcheck : {}})

    const interval = opts.healthcheck.interval
      ? opts.healthcheck.interval >= 1000
        ? opts.healthcheck.interval
        : 1000
      : 5000

    const timeout = opts.healthcheck.timeout
      ? opts.healthcheck.timeout < interval
        ? opts.healthcheck.timeout
        : Math.trunc(interval * 0.9)
      : 4000
    ...
    return Object.assign(
      ...
      opts,
      {
        healthcheck: {
          ...
          interval,
          timeout,
          healthyThreshold: opts.healthcheck.healthyThreshold || 2,
          unhealthyThreshold: opts.healthcheck.unhealthyThreshold || 2
        }
      },
      rewrite ? {rewrite} : {}
    )
