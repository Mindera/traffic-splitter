...
const { InternalError } = require('restify')
...
module.exports = (httpClient, httpAgent) =>
  (req, res, next, { config, eventEmitter, logger, bidCookieDetails }) => {
    ...
    proxy.on('error', (err) => {
      logger.error({
        message: `Error while serving ${req.url}: ${err}`,
        upstream: {
      ...
      return next(new InternalError(`Piping error: ' ${err}`))
    })
