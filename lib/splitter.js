...
module.exports.isConfigurationValid = (configuration) => config.validate(configuration)
...
function Splitter (configuration) {
  ...
  const splitterLogger = logger.init(configuration)
  const bootstrapMiddlewares = []
  const requestMiddlewares = []
  const rules = {}
  const executors = {}
  const started = () => splitterLogger.error('This splitter instance has already started')
  let server
